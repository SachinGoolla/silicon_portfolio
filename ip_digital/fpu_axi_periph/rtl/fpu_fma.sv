...
module fpu_fma #(
    ...
    always_comb begin
        if (s1_is_add_op) begin
            // Place mant_a in upper half; lower half = 0 (alignment done in EX2)
            // Leading 0 keeps hidden bit at bit PMANT_W-2 → overflow flag stays 0
    ...
    // =========================================================================
    // EX3a: Signed add  (29-bit carry chain — timing bottleneck)
    // EX3b: Leading-zero detect + zero-flag  (priority encoder on r2b_mag)
    //
    // Split into two sub-stages to cut the ~12.75 ns EX3 path in half and
    // close 80 MHz TT timing.  FMA latency: 5 → 6 cycles (throughput unchanged).
    //
    ...
    // EX3a → EX3b pipeline register (captures adder result; LZD runs next cycle)
    logic                 r2b_valid;
    logic [2:0]           r2b_rm;
    logic                 r2b_sign;
    logic signed [EW+2:0] r2b_exp_result;
    logic [MAGW-1:0]      r2b_mag;
    logic                 r2b_sticky;
    logic                 r2b_result_is_nan, r2b_result_is_inf, r2b_result_is_zero;
    logic                 r2b_set_nv, r2b_inf_sign, r2b_zero_sign;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            r2b_mag            <= '0;
            r2b_sticky         <= 1'b0;
            r2b_result_is_nan  <= 1'b0;
            r2b_result_is_inf  <= 1'b0;
            r2b_result_is_zero <= 1'b0;
            r2b_set_nv         <= 1'b0;
            r2b_inf_sign       <= 1'b0;
            r2b_zero_sign      <= 1'b0;
        end else begin
            ...
            r2b_rm             <= r2_rm;
            r2b_sign           <= s3_sign;
            r2b_exp_result     <= r2_exp_result;
            r2b_mag            <= s3_mag;
            r2b_sticky         <= s3_sticky;
            r2b_result_is_nan  <= r2_result_is_nan;
            r2b_result_is_inf  <= r2_result_is_inf;
            r2b_result_is_zero <= r2_result_is_zero;
            r2b_set_nv         <= r2_set_nv;
            r2b_inf_sign       <= r2_inf_sign;
            r2b_zero_sign      <= r2_zero_sign;
        end
    end

    // EX3b combinational: LZD + zero-detect on registered adder result
    logic s3b_is_zero;
    assign s3b_is_zero = (r2b_mag == '0);

    logic [LZDW-1:0] s3b_lzd;
    always_comb begin
        s3b_lzd = LZDW'(MAGW);
        for (int i = 0; i < MAGW; i++) begin
            if (r2b_mag[i]) s3b_lzd = LZDW'(MAGW - 1 - i);
        end
    end

    // EX3b → EX4 pipeline register
    logic                 r3_valid;
    logic [2:0]           r3_rm;
    logic                 r3_sign;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            r3_rm             <= r2b_rm;
            r3_sign           <= r2b_sign;
            r3_exp_result     <= r2b_exp_result;
            r3_mag            <= r2b_mag;
            r3_sticky         <= r2b_sticky;
            r3_lzd            <= s3b_lzd;
            r3_is_zero        <= s3b_is_zero;
            r3_result_is_nan  <= r2b_result_is_nan;
            r3_result_is_inf  <= r2b_result_is_inf;
            r3_result_is_zero <= r2b_result_is_zero | s3b_is_zero;
            r3_set_nv         <= r2b_set_nv;
            r3_inf_sign       <= r2b_inf_sign;
            r3_zero_sign      <= r2b_zero_sign;
        end
    end
    ...
    // r3_mag in UPPER half so its MSB aligns to bit SMAGW-1 before shifting

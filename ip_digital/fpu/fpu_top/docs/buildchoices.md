...
### Result
```
P3: PASS — 24/24 tests (20 cocotb directed + 4 pyUVM; 126 scoreboard checks)
```
...
**Tool:** Verilator `--binary --coverage --coverage-toggle --coverage-expr --trace-fst`
...
Verilator inserts three types of coverage with the current P4 flags:

- **Line coverage** (`--coverage`): which `always` block lines executed
- **Toggle coverage** (`--coverage-toggle`): each signal driven both 0→1 and 1→0
- **Expression/branch coverage** (`--coverage-expr`): each conditional branch taken

For fpu_top, line and expression coverage are high for FMA/noncomp/cvt paths.
Toggle coverage is lower for `int_mag` and `i2f_mag` deep bits in `fpu_cvt.sv`
(require large integer values to exercise — covered by directed vectors).
Coverage is lowest for divsqrt extreme-corner paths (max iteration, div-by-zero
overflow) — expected for a directed test suite without constrained-random stimulus.

### Expression Coverage Parsing (P5)

`p5_coverage.py` parses `v_expr` entries from `coverage.dat` (written only when
`--coverage-expr` is passed to P4). Each line is counted as covered if its hit
count > 0. Testbench lines are filtered out by matching `verif_dir.name`.
Result stored in `flow.all_metrics.coverage.branch_coverage_pct`.

### Advisory vs hard gate

Coverage is advisory by default because percentage alone is imperfect — 100%
toggle of a buggy design still fails functional tests. For CI enforcement:
```
pillar --top fpu_top --coverage-threshold 90   # hard gate on line coverage
```

### Result
```
P5: PASS — Line 95% (686/716 lines)  Toggle 76.8%  Expr 79.7%
          30 genuinely unreachable lines (divsqrt extreme-corner paths,
          fpu_round subnormal-to-normal, default rounding mode arms)
```
...
Corner                    Slack        Status
──────────────────────────────────────────────
ff_n40C_1v76  (fast)     +2.861 ns    MET  (22.9% margin)
tt_025C_1v80  (typical)  +0.131 ns    MET  ← primary sign-off corner
ss_n40C_1v28  (slow)    −84.202 ns   advisory extreme corner
```

The SS corner at -40°C / 1.28V is the most pessimistic operating condition.
The −84.202 ns gap is real pre-layout pessimism (no back-annotation); at this
extreme corner the device would need a much lower frequency or a PnR-optimized
netlist. For this portfolio FPU the SS result is advisory — not a hard gate.

The SS result improved from the initial −92.593 ns to −84.202 ns after
`_filter_liberty_lpflow()` removed 34 lpflow power-gating cells from the
liberty file before abc mapping (they were adding ~8-9 ns of spurious derating).

The TT corner (+0.131 ns) is the primary sign-off point. It passes.

### Gate-Level Simulation (GLS)

GLS re-runs all cocotb tests driving the synthesized gate netlist
(`fpu_top_synth.v`) instead of RTL. This catches:
...
Icarus needs the sky130 behavioral Verilog cell models (`sky130_fd_sc_hd.v`).
If the file is missing, GLS returns SKIP (environment issue, not design issue).
`sky130_fd_sc_hd.v` is a symlink at `lib/sky130_fd_sc_hd.v` → system PDK install.

### Result
```
P8: PASS — TT +0.131 ns MET; FF +2.861 ns MET; SS −84.202 ns advisory
           GLS PASS (sky130 zero-delay Icarus; all tests PASS)
```
...
P3 Functional  PASS  — 24/24 tests (20 cocotb + 4 pyUVM; 126 scoreboard checks)
P4 Simulation  PASS  — no $error/$fatal in Verilator binary
P5 Coverage    PASS  — Line 95%  Toggle 76.8%  Expr 79.7%
P6 Synthesis   PASS  — 1,004 sky130 cells (ceiling 1,200)
P7 LEC         PASS  — Yosys miniSAT, 5 combinational modules proven
P8 STA+GLS     PASS  — TT +0.131 ns MET, FF +2.861 ns MET, SS advisory
                        GLS PASS (sky130 zero-delay Icarus)

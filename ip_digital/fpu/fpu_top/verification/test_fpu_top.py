...
def fclass_golden(a: int) -> int:
    ...
    Bit layout (RISC-V ISA §11.2):
      ...
      [0] -inf        [1] -normal  [2] -subnormal  [3] -zero
      [4] +zero       [5] +subnormal  [6] +normal   [7] +inf
    """
    s    = sign_of(a)
    exp  = (a >> 23) & 0xFF
    mant = a & 0x7FFFFF
    ...
    if is_inf_ and     s:     return 1 << 0   # -infinity
    if is_norm_ and    s:     return 1 << 1   # -normal
    if is_sub_ and     s:     return 1 << 2   # -subnormal
    if is_zero_ and    s:     return 1 << 3   # -zero
    if is_zero_ and not s:    return 1 << 4   # +zero
    if is_sub_ and  not s:    return 1 << 5   # +subnormal
    if is_norm_ and not s:    return 1 << 6   # +normal
    if is_inf_ and  not s:    return 1 << 7   # +infinity
...
async def test_fclass(dut):
    ...
    vectors = [
        (FP32_QNAN,      1 << 9, "qNaN"),
        (FP32_SNAN,      1 << 8, "sNaN"),
        (FP32_NEG_INF,   1 << 0, "-inf"),
        (FP32_NEG_ONE,   1 << 1, "-normal"),
        (FP32_NEG_SMALL, 1 << 2, "-subnormal"),
        (FP32_NEG_ZERO,  1 << 3, "-zero"),
        (FP32_POS_ZERO,  1 << 4, "+zero"),
        (FP32_POS_SMALL, 1 << 5, "+subnormal"),
        (FP32_POS_ONE,   1 << 6, "+normal"),
        (FP32_POS_INF,   1 << 7, "+inf"),
        (FP32_NEG_PI,    1 << 1, "-π (normal)"),
        (FP32_POS_PI,    1 << 6, "+π (normal)"),
        (FP32_POS_MAX,   1 << 6, "+MAX_NORMAL"),
        (FP32_NEG_MAX,   1 << 1, "-MAX_NORMAL"),
    ]
...
# FMA pipeline latency — updated after EX3 pipeline split (r2b register added):
# valid_i asserted before loop; valid_o=1 observed on cycle 5 (6th clock edge).
# Loop runs LATENCY-1 = 5 extra edges after the first.
FMA_LATENCY = 6
...
async def test_fma_pipeline(dut):
    ...
    Each op takes 6 cycles (5 pipeline stages + output register).
